...
void InputData(data *dt[100][100]){
    for (int i = 0; i < baris; i++)
    {
        for (int j = 0; j < kolom; j++)
        {
            ...
            *dt[i][j]->rataan = (*dt[i][j]->nilai[0] + *dt[i][j]->nilai[1] + *dt[i][j]->nilai[2]) / 3;
            *dt[i][j]->rataan = (float)(*dt[i][j]->rataan);
        }
    }
}
        ...
        for (int i = 0; i < baris; i++)
        {
            for (int j = 0; j < kolom; j++)
            {
                ...
                if(sama == 2)
                {
                    ...
                    {
                        Swap(dt[idxB[a-1]][idxK[a-1]]->nama, dt[idxB[a]][idxK[a]]->nama, idxB[a-1], idxK[a-1], idxB[a], idxK[a], &dt);
                    }
                }
            }
        }
